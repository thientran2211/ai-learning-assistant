...
const QuizCard = ({ quiz, onDelete }) => {
  ...
  return (
    <div className="group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-emerald-300 rounded-xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/10 flex flex-col justity-between">
      <button
        onClick={(e) => {
          e.stopPropagation();
          onDelete(quiz);
      ...
      <div className="space-y-4">
        ...
        <div className="inline-flex items-center gap-1.5 py-1 rounded-lg text-xs font-semibold">
          <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-1 ">
            ...
            <span className="text-emerald-700">{t('quizzes.score')} {quiz?.score}</span>
          </div>
        </div>
        ...
        <div>
          ...
          >
            {quiz.title ||
              `${t('quizzes.quizTitle')} - ${formatDateUtil(quiz.createdAt)}`}
          </h3>
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-slate-100">
          <div className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
            <span className="text-sm font-semibold text-slate-700">
              ...
              {quiz.questions.length === 1 ? t('quizzes.questionSingular') : t('quizzes.questionPlural')}
            </span>
          </div>
        </div>
      </div>
