        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-900 mt-1">
              ...
              {flashcardSets.length === 1 ? t('flashcards.setSingular') : t('flashcards.setPlural')} {t('flashcards.available')}
            </p>
          </div>
          <button
            onClick={handleGenerateFlashcards}
              ...
              <div className="space-y-4">
                ...
                <div>
                  <h4 className="text-base font-semibold text-slate-900 mb-1">
                    {t('flashcards.setTitle')}
                  </h4>
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">
                    {t('flashcards.created')} {formatDateUtil(set.createdAt)}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                  <div className="px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <span className="text-sm font-semibold text-emerald-700">
                      {set.cards.length}{" "}
                      {set.cards.length === 1 ? t('flashcards.cardSingular') : t('flashcards.cardPlural')}
                    </span>
                  </div>
                </div>
              </div>
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={t('flashcards.modalDeleteTitle')}
      >
        <div className="space-y-6">
          <p className="text-sm text-slate-600">
             {t('flashcards.modalDeleteDesc')}
          </p>
            ...
            >
              {deleting ? (
                <span className="flex items-center gap-2">
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  {t('flashcards.deleting')}
                </span>
              ) : (
                t('flashcards.btnDeleteSet')
              )}
            </button>
          </div>
        </div>
